...
postcondition_common(_S, {call, ?MODULE, mine, _Args, _}, {A, B}) ->
   aec_trees:hash(A) == aec_trees:hash(B) orelse eq(A,B);
postcondition_common(S, {call, ?MODULE, Fun, Args, _}, Res) ->
...
add_instructions_next(S, _, [Instructions]) ->
    maps:update_with(instructions, fun(Old) -> {call, lists, append, [Old, Instructions]} end, S).
...
mine_args(#{height := Height} = S) ->
    [Height, maps:get(instructions, S, [])].

mine_pre(#{height := Height, instructions := Instructions}, [H, Inst]) ->
    Height == H andalso Instructions == Inst.

mine_adapt(#{height := Height, instructions := Instructions}, [_, _]) ->
    [Height, Instructions];
mine_adapt(_, _) ->
    false.

mine(Height, Instructions) ->
...
mine_next(S, Value, [H, _]) ->
    multi_mine_next(S, Value, [H, 1]).


mine_features(S, [H, Inst], _Res) ->
    [mine_response_ttl || expired_queries(S, H) =/= [] ] ++
    [{mine_instructions, length(Inst)}] ++
    [mine].
...
multi_mine_pre(S) ->
    maps:is_key(accounts, S) andalso maps:get(instructions, S, []) == [].
    ...
    ?LET({{SenderTag, Sender}, {ReceiverTag, Receiver}},
         ...
         ?LET([Amount, Nonce, To],
              ...
              [Height, {SenderTag, Sender#account.key},
               ...
               #{sender_id => aser_id:create(account, Sender#account.key),  %% The sender is asserted to never be a name.
                 recipient_id =>
                     case To of
                         account ->
                             aser_id:create(account, Receiver#account.key);
                         {name, Name} ->
                             aser_id:create(name, aens_hash:name_hash(Name))
                     end,
                 amount => Amount,
                 fee => gen_fee(Height),
                 nonce => Nonce,
...
spend_callouts(S, Args) ->
    ?APPLY(instructions, [spend_valid(S, Args)]).
...
register_oracle_args(S = #{height := Height}) ->
     ?LET({SenderTag, Sender}, gen_new_oracle_account(S),
          [Height, {SenderTag, Sender#account.key},
                #{account_id => aser_id:create(account, Sender#account.key),
...
register_oracle_callouts(S, Args) ->
    ?APPLY(instructions, [register_oracle_valid(S, Args)]).
...
extend_oracle_args(S = #{height := Height}) ->
    ?LET({{_, Oracle}, DeltaTTL},
         ...
         [Height, Oracle#account.key,
          #{oracle_id  => aser_id:create(oracle, Oracle#account.key),
...
extend_oracle_callouts(S, Args) ->
    ?APPLY(instructions, [extend_oracle_valid(S, Args)]).
     ...
     ?LET({{SenderTag, Sender}, {_, Oracle}},
          ...
          begin
              ...
              [Height, {SenderTag, Sender#account.key}, Oracle#account.key,
               #{sender_id => aser_id:create(account, Sender#account.key),
                 oracle_id => aser_id:create(oracle, Oracle#account.key),
...
query_oracle_callouts(S, Args) ->
    ?APPLY(instructions, [query_oracle_valid(S, Args)]).
...
response_oracle_args(#{accounts := Accounts, height := Height} = S) ->
     ?LET({Sender, Nonce, Oracle} = QueryId,
          ...
          [Height, {Sender, Nonce, Oracle},
           #{oracle_id => aser_id:create(oracle, Oracle),
...
response_oracle_callouts(S, Args) ->
    ?APPLY(instructions, [response_oracle_valid(S, Args)]).
...
channel_create_args(#{accounts := Accounts, height := Height}) ->
     ...
     ?LET({IAmount, RAmount, ChannelReserve}, gen_create_channel_amounts(Height),
          [Height, Initiator#account.key, Responder#account.key,
                #{initiator_id => aser_id:create(account, Initiator#account.key),
                  responder_id => aser_id:create(account, Responder#account.key),
                  state_hash => <<1:256>>,
...
channel_create_callouts(S, Args) ->
    ?APPLY(instructions, [channel_create_valid(S, Args)]).
...
channel_deposit_args(#{height := Height} = S) ->
     ...
     begin
          ...
          [Height, {Initiator, N, Responder}, Party,
                #{channel_id => aser_id:create(channel, aesc_channels:pubkey(Initiator, N, Responder)),
                  from_id => aser_id:create(account, From),
...
channel_deposit_callouts(S, Args) ->
    ?APPLY(instructions, [channel_deposit_valid(S, Args)]).
...
channel_withdraw_args(#{height := Height} = S) ->
    ...
    begin
         ...
         [Height, {Initiator, N, Responder}, Party,
               #{channel_id => aser_id:create(channel, aesc_channels:pubkey(Initiator, N, Responder)),
                 to_id => aser_id:create(account, From),
                 amount => gen_channel_amount(Height),
                 round => gen_channel_round(S, CId),
                 fee => gen_fee(Height),
                 state_hash => <<0:256>>,
...
channel_withdraw_callouts(S, Args) ->
    ?APPLY(instructions, [channel_withdraw_valid(S, Args)]).
...
channel_close_mutual_callouts(S, Args) ->
    ?APPLY(instructions, [channel_close_mutual_valid(S, Args)]).
...
ns_preclaim_pre(S, [Height, {_, Sender}, {Name, Salt}, Tx]) ->
    ...
    [present || #preclaim{name = N, salt = St} <- maps:get(preclaims, S, []), N == Name, St == Salt] == []
        andalso aser_id:create(commitment, aens_hash:commitment_hash(Name, Salt)) == maps:get(commitment_id, Tx)
...
ns_preclaim_callouts(S, Args) ->
    ?APPLY(instructions, [ns_preclaim_valid(S, Args)]).
...
ns_claim_callouts(S, Args) ->
    ?APPLY(instructions, [ns_claim_valid(S, Args)]).
...
ns_update_args(#{accounts := Accounts, height := Height} = S) ->
     ?LET({{Name, {SenderTag, Sender}}, {Tag, NameAccount}},
          ...
          [Height, Name, {SenderTag, Sender#account.key}, {Tag, NameAccount#account.key},
           #{account_id => aser_id:create(account, Sender#account.key),
             name_id => aser_id:create(name, aens_hash:name_hash(Name)),
             name_ttl => frequency([{10, nat()}, {1, 36000}, {10, 25000}, {1, choose(30000, 60000)}]),
             client_ttl => nat(),
             fee => gen_fee(Height),
             nonce => gen_nonce(Sender),
             pointers =>
                 gen_pointers(Accounts, aser_id:create(account, NameAccount#account.key))
            }]).
    ...
    frequency([
        ...
        {1, [aens_pointer:new(<<"account_pubkey">>, Id),
             ?LET({_, Acc}, gen_account(1, 49, Accounts),
                  aens_pointer:new(<<"account_pubkey">>, aser_id:create(account, Acc#account.key)))]},
...
ns_update_pre(S, [Height, Name, {_, Sender}, _NameAccount, Tx]) ->
    aser_id:create(name, aens_hash:name_hash(Name)) == maps:get(name_id, Tx)
        andalso correct_height(S, Height) andalso valid_nonce(S, Sender, Tx).
...
ns_update_callouts(S, Args) ->
    ?APPLY(instructions, [ns_update_valid(S, Args)]).
...
ns_revoke_callouts(S, Args) ->
    ?APPLY(instructions, [ns_revoke_valid(S, Args)]).
    ...
    ?LET({{Name, {SenderTag, Sender}}, {ReceiverTag, Receiver}},
         ...
         ?LET(To, oneof([account, {name, elements(maps:keys(maps:get(named_accounts, S, #{})) ++ [<<"ta.test">>])}]),
              [Height, {SenderTag, Sender#account.key},
               ...
               #{account_id => aser_id:create(account, Sender#account.key),  %% The sender is asserted to never be a name.
                 recipient_id =>
                     case To of
                         account ->
                             aser_id:create(account, Receiver#account.key);
                         {name, ToName} ->
                             aser_id:create(name, aens_hash:name_hash(ToName))
                     end,
                 name_id => aser_id:create(name, aens_hash:name_hash(Name)),
                 fee => gen_fee(Height),
                 nonce => gen_nonce(Sender)
...
ns_transfer_callouts(S, Args) ->
    ?APPLY(instructions, [ns_transfer_valid(S, Args)]).
...
contract_create_callouts(S, Args) ->
    ?APPLY(instructions, [contract_create_valid(S, Args)]).
...
contract_call_callouts(S, Args) ->
    ?APPLY(instructions, [contract_call_valid(S, Args)]).
...
prop_txs() ->
    ...
    begin
        ...
        check_command_names(Cmds,
            ...
            aggregate_feats([atoms, correct, mine_instructions, protocol | all_command_names()], call_features(H),
...
correct_name_id(Name, NameId) ->
    aser_id:create(name, aens_hash:name_hash(Name)) == NameId.
